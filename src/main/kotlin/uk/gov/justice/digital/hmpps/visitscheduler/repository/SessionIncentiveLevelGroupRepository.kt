package uk.gov.justice.digital.hmpps.visitscheduler.repository

import org.springframework.data.jpa.repository.JpaRepository
import org.springframework.data.jpa.repository.Modifying
import org.springframework.stereotype.Repository
import uk.gov.justice.digital.hmpps.visitscheduler.model.entity.session.incentive.SessionIncentiveLevelGroup

@Repository
interface SessionIncentiveLevelGroupRepository : JpaRepository<SessionIncentiveLevelGroup, Long> {
  fun findByReference(reference: String): SessionIncentiveLevelGroup?
  fun findByPrisonCode(prisonCode: String): List<SessionIncentiveLevelGroup>

  @Modifying
  fun deleteByReference(reference: String): Int
}
