...
import uk.gov.justice.digital.hmpps.visitscheduler.utils.PrisonerWingParser
...
class PrisonApiService(
  ...
  fun getPrisonerWing(prisonerId: String): String? {
    ...
    try {
      ...
      prisonerDetails?.let { prisonerDetailList ->
        if (prisonerDetailList.isNotEmpty()) {
          // there should only be 1 value in prisonerDetailList as we asked for only 1 offenders details
          return PrisonerWingParser.getPrisonerWingFromLocation(prisonerDetailList[0])
        }
      }
