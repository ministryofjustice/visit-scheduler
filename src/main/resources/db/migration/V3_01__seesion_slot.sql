...
INSERT INTO session_slot_no_order (session_template_reference,prison_id,slot_date,slot_start,slot_end)
    SELECT st.reference AS session_template_reference,
        ...
        GROUP BY st.reference,v.visit_start,v.visit_end ,st.start_time,st.end_time,v.prison_id
...
INSERT INTO session_slot (reference,session_template_reference,prison_id,slot_date,slot_start,slot_end)
        SELECT REGEXP_REPLACE(to_hex(id+2951597050), '(.{3})(?!$)', '\1~','g'),session_template_reference,prison_id,slot_date,slot_start,slot_end FROM session_slot_no_order ORDER BY slot_date,slot_start;
