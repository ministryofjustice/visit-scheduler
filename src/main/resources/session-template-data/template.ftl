...
<#list sessionRecords as s>
    (<#if s.locationKeys??>'${s.locationKeys}'<#else>NULL</#if>,'${s.prison}','${s.room}','${s.type}',${s.open},${s.closed},'${s.startTime}','${s.endTime}','${s.startDate}',<#if s.endDate??>'${s.endDate}'<#else>NULL</#if>,'${s.dayOfWeek}',${s.biWeekly?string})<#if s_has_next>,</#if>
</#list>;
