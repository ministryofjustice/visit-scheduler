    ...
    <#list prisonCodes as pc>
        ...
        <#list sessionRecords as s>
            (<#if s.locationKeys??>'${s.locationKeys}'<#else>NULL</#if>,<#if s.categoryKeys??>'${s.categoryKeys}'<#else>NULL</#if>,'${s.prisonCode}','${s.room}','${s.type}',${s.open},${s.closed},${s.enhanced?string},'${s.startTime}','${s.endTime}','${s.startDate}',<#if s.endDate??>'${s.endDate}'<#else>NULL</#if>,'${s.dayOfWeek}',${s.biWeekly?string},'${s.dayOfWeek}, ${s.startDate}, ${s.startTime}')<#if s_has_next>,</#if>
        </#list>;
