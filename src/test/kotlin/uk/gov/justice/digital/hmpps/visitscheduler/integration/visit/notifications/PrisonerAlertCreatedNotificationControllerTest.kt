...
import uk.gov.justice.digital.hmpps.visitscheduler.dto.enums.EventAuditType.PRISONER_ALERT_CREATED_EVENT
...
import uk.gov.justice.digital.hmpps.visitscheduler.helper.callNotifyVSiPThatPrisonerAlertHasBeenCreated
...
@DisplayName("POST $VISIT_NOTIFICATION_PRISONER_ALERT_CREATED_PATH")
class PrisonerAlertCreatedNotificationControllerTest : NotificationTestBase() {
  ...
  @Test
  fun `when prisoner has had an alert created then only associated future booked visits for the prisoner are flagged`() {
    // Given
    ...
    // When
    val responseSpec = callNotifyVSiPThatPrisonerAlertHasBeenCreated(
      webTestClient,
    ...
    val auditEvents = testEventAuditRepository.getAuditByType(PRISONER_ALERT_CREATED_EVENT)
    assertThat(auditEvents).hasSize(1)
    assertAuditEvent(auditEvents[0], visit = visit1, eventType = PRISONER_ALERT_CREATED_EVENT)

    assertFlaggedVisitEvent(listOf(visit1), NotificationEventType.PRISONER_ALERT_CREATED_EVENT)
    verify(telemetryClient, times(1)).trackEvent(eq("flagged-visit-event"), any(), isNull())
  }

  @Test
  fun `when prisoner has had an alert created then associated future booked visits across prisons for the prisoner are flagged`() {
    // Given
    val notificationDto = PrisonerAlertNotificationDto(
      prisonerNumber = prisonerId,
      alertCode = "C1",
    ...
    assertAuditEvent(auditEvents[0], visit = visit1, eventType = PRISONER_ALERT_CREATED_EVENT)
    assertAuditEvent(auditEvents[1], visit = visit2, eventType = PRISONER_ALERT_CREATED_EVENT)

    assertFlaggedVisitEvent(listOf(visit1, visit2), NotificationEventType.PRISONER_ALERT_CREATED_EVENT)
    verify(telemetryClient, times(2)).trackEvent(eq("flagged-visit-event"), any(), isNull())
  }

  @Test
  fun `when prisoner has had an alert created but no future booked visits then no visits are flagged`() {
    // Given
    val notificationDto = PrisonerAlertNotificationDto(
      prisonerNumber = prisonerId,
      alertCode = "C1",
