...
/*  @Test
  fun `prisoner wing details are returned for BLI prisoner when wing exists`() {
    val prisonerId = "AA1234BB"
    val prisonId = "BLI"
    val internalLocation = "$prisonId-WING-HOUSE-CELL"
    ...
    // When
    val prisonerWing = prisonApiService.getPrisonerWing(prisonerId)

    // Then
    assertThat(prisonerWing).isEqualTo("WING")
  }

  @Test
  fun `prisoner wing details is returned as null for BLI when internal location in wrong format`() {
    val prisonerId = "AA1234BB"
    val prisonId = "BLI"
    val internalLocation = "$prisonId-WING-CELL"
    ...
    // Then
    assertThat(prisonerWing).isNull()
  }

  @Test
  fun `prisoner wing details is returned as null for BLI when internal location in not delimited`() {
    val prisonerId = "AA1234BB"
    val prisonId = "BLI"
    val internalLocation = "${prisonId}WINGCELL"
  ...
  @Test
  fun `prisoner wing details is returned as null for non BLI prisoner`() {
    val prisonerId = "AA1234BB"
    val prisonId = "HEI"
    val internalLocation = "$prisonId-WING-HOUSE-CELL"
  ...
  @Test
  fun `prisoner wing is null when prison api get prisoner details call returns NOT FOUND`() {
    val prisonerId = "AA1234BB"
  ...
  @Test
  fun `get prisoner wing throws WebClientResponseException for BAD REQUEST`() {
    val prisonerId = "AA1234BB"
    ...
    // When
    assertThrows<WebClientResponseException> {
      prisonApiService.getPrisonerWing(prisonerId)
    }
