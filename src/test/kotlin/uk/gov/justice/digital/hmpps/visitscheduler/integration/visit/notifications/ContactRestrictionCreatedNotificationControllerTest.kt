...
class ContactRestrictionCreatedNotificationControllerTest : NotificationTestBase() {
  ...
  fun `when a contact global restriction is created then valid visits in all prisons flagged and saved`() {
    // Given
    val notificationDto = ContactRestrictionCreatedNotificationDto(
      visitorId = visitorId.toLong(),
      restrictionId = visitorRestrictionId.toLong(),
    )

    val contactRestrictions = listOf(
      ContactRestrictionDto(contactRestrictionId = visitorRestrictionId.toLong(), contactId = visitorId.toLong(), restrictionType = VisitorSupportedRestrictionType.BAN.name, startDate = LocalDate.now()),
    )
    ...
    // When
    personalRelationshipsApiMockServer.stubGetContactRestrictions(visitorId.toLong(), contactRestrictions)
    val responseSpec =
      callNotifyVSiPThatContactRestrictionCreated(webTestClient, roleVisitSchedulerHttpHeaders, notificationDto)
  ...
  @Test
  fun `when a contact global restriction is created but cannot be found in relationship API then no visits are flagged or saved`() {
    // Given
  ...
  fun `when a contact global restriction is created with a date in the past then no visits are flagged or saved`() {
    ...
    val contactRestrictions = listOf(
      ContactRestrictionDto(contactRestrictionId = visitorRestrictionId.toLong(), contactId = visitorId.toLong(), restrictionType = VisitorSupportedRestrictionType.CLOSED.name, startDate = LocalDate.now()),
    )
  ...
  @Test
  fun `when a contact global restriction is created with expiry date then no visits after expiry are flagged or saved`() {
    // Given
    val notificationDto = VisitorRestrictionUpsertedNotificationDto(
      visitorId = visitorId,
      validFromDate = LocalDate.now().minusDays(2),
